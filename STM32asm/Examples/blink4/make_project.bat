...
for /r /d %%i in (*) do (
   ...
   for %%b in (*.asm) do (
      ...
      echo !a!
      cd !pth!
%path%arm-none-eabi-as.exe -o !o! !a!
%path%arm-none-eabi-objdump.exe -j .vectors -j .asmcode -j .bss -j .rodata -d -t -w !o! > !l!
      cd %%i
   )
)
...
cd %pth%
%path%arm-none-eabi-ld.exe -T src\stm32f40_map.ld -o compile\sys.elf %ofiles%

...
LDI 	R16, 0x21 ; старший байт конечного адреса ОЗУ в R16 
STS 	0x5e, R16 ; установка старшего байта указателя стека SPH
; -- устанавливаем пин PB7 порта B на выход -- 
SBI 	0x04, 7 ; загрузим значение из регистра R16 в порт DDRB 
; -- основной цикл программы -- 
Main: 	
	SBI 	0x05, 7 ; подача на пин PB7 высокого уровня 
	RCALL 	Wait ; вызываем функцию задержки по времени 
	CBI 	0x05, 7 ; подача на пин PB7 низкого уровня 
	RCALL 	Wait ; вызываем функцию задержки по времени 	
